...
export default function TesteEquipeSection({ colaboradores, onSelectColaborador }: TesteEquipeSectionProps) {
  ...
  const { data: todosPerfil = [] } = useQuery({
    ...
    enabled: colaboradores.length > 0 && aberto,
  });
      ...
      <Card>
        ...
        >
          <CardTitle className="flex items-center justify-between text-base">
            ...
            <ChevronDown
              className={`h-4 w-4 text-muted-foreground transition-transform duration-200 ${aberto ? "rotate-180" : ""}`}
            />
          </CardTitle>
        </CardHeader>
        ...
        {aberto && (
          <CardContent className="p-0">
            <div className="rounded-b-xl overflow-hidden overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    return (
                      <TableRow key={colab.id}>
                        <TableCell
                          className={
                            onSelectColaborador
                              ? "font-medium cursor-pointer hover:underline text-primary"
                              : "font-medium"
                          }
